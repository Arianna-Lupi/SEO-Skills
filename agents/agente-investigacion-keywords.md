...
- **Ahrefs MCP (PAGO):** `keywords-explorer-overview` (volumen, KD, tráfico potencial), `-matching-terms` y `-related-terms` (expansión), `-search-suggestions` (long-tail), `serp-overview` (competencia que rankea). Úsalo para llenar columnas de métricas con datos reales. Detalle de conexión en `../MCP-SETUP.md`. Nunca exijas un MCP; si falta, pide el export y continúa.
...
2. **Expandir con los 4 métodos del diploma**, en este orden:
   ...
   - *IA* — solo para generar candidatas; quedan marcadas como "sin validar". Reúne mínimo 30 keystrokes/keywords candidatas (objetivo del diploma).
...
- (Opcional) si me piden persistir, guardo el entregable en `Write` como `.md` en la ruta que indiquen. No devuelvo cadenas de razonamiento: entrego la tabla final y las decisiones, no el proceso intermedio.
...
- `keywords.json` (candidates + 10 de Oro), `clusters.json` y, si leíste SERPs, `competitors.json`.
